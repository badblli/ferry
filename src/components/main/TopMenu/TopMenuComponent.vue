              ...
              <div ref="dropdown">
                ...
                <transition name="dropdown">
                  <div v-show="isOpen" id="lang" class="absolute z-50 top-12 w-64 bg-white rounded-xl border transition-[opacity,margin] duration-300 mt-2 min-w-[15rem] p-7">
                    <a v-for="(language, langIndex) in languages" :key="langIndex" @click="updateCurrentLanguage(language); handleClickInside()" class="cursor-pointer flex items-center gap-x-3.5 py-[9px] px-[14px] mb-[9px] text-black text-base font-medium font-display tracking-tight rounded-lg hover:bg-slate-200 focus:outline-none">
                      {{ language.name }}
                    </a>
                  </div>
                </transition>

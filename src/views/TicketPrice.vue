                    ...
                    <div className="flex flex-col overflow-x-auto">
                         <div className="p-8">
                              ...
                              <table className="relative bg-white mb-4 rounded-2xl w-full mt-10">
                                   <thead>
                                        <tr v-for="(header, index) in tableHeaders" :key="index" class="md:mx-24 mx-14 mt-8 md:mt-14 md:mb-9 flex flex-row justify-between text-black md:text-lg text-base font-semibold leading-loose rounded-2xl">
                                             <th v-for="(label, labelIndex) in header" :key="labelIndex" class="md:pl-14 first:pl-2 w-full flex">{{ label }}</th>
                                        </tr>
                                   </thead>
                                   <tbody>
                                        <tr v-for="(row, index) in tableData" :key="index" className="flex flex-row mx-16 mb-[15px] last:mb-[70px] border border-stone-300 rounded-2xl [&>*:nth-last-child(1)]:border-none">
